...
func tryPayment(
	...
	for try := 0; try < WaitPaymentMaxAttempts; try++ {
		...
		if err != nil {
			...
			if cmderr, ok := err.(ErrorCommand); ok {
				switch cmderr.Code {
				case 204:
					// error in route, try another one
	...
	retry:
		...
		ichannel, ok1 := cmderrData["erring_channel"]
		...
		direction := getChannelDirectionInRoute(route, channel)
...
func getChannelDirectionInRoute(route gjson.Result, channel string) (direction int64) {
